...
Do not claim test execution, browser behavior, accessibility validation, performance measurements, or production behavior without direct evidence. When a relevant concern cannot be checked from the available diff, test output, or configured tooling, write `Not verified:` and name the missing check. This is context for the author, not an actionable defect.
